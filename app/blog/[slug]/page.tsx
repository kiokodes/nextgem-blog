...
const ptComponents = {
  marks: {
    link: ({ children, value }: any) => {
      ...
      return (
        <a href={value?.href} target={isExternal ? '_blank' : undefined}
  ...
  block: {
    h2: ({ children }: any) => <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(22px,3vw,32px)', fontWeight: 900, margin: '40px 0 16px', color: '#1a1a1a' }}>{children}</h2>,
    h3: ({ children }: any) => <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 700, margin: '32px 0 12px' }}>{children}</h3>,
    blockquote: ({ children }: any) => <blockquote style={{ borderLeft: '4px solid #1D56E8', padding: '16px 24px', background: '#eef3fd', borderRadius: '0 8px 8px 0', fontStyle: 'italic', color: '#555', margin: '28px 0' }}>{children}</blockquote>,
    normal: ({ children }: any) => <p style={{ marginBottom: 22 }}>{children}</p>,
  },
  types: {
    image: ({ value }: any) => {
      if (!value?.asset?._ref) return null
      const imgId = value.asset._ref.replace('image-', '').replace(/-jpg$/, '.jpg').replace(/-png$/, '.png').replace(/-webp$/, '.webp')
      return <img src={`https://cdn.sanity.io/images/6l4myqih/production/${imgId}?w=740&auto=format`} alt={value.alt || ''} loading="lazy" style={{ borderRadius: 10, margin: '32px 0', width: '100%' }} />
    },
  },
}
...
export default function BlogPostPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    ...
    client.fetch(`*[_type == "post" && slug.current != $slug] | order(publishedAt desc) [0..3] {
  ...
  if (!post) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'sans-serif', color: '#555' }}>
      Loading...
    </div>
  )
    ...
    <>
      ...
      <div className="article-wrap">
        <main className="article-main">
          ...
          {post.categoryTitle && <span className="article-category">{post.categoryTitle}</span>}
          ...
          {!donateClosed && (
            <div className="donate-popup-card">
              ...
              <p className="popup-subtitle">
                You&apos;re making a <span>{popupFreq}</span> donation of <span>₦{popupAmount.toLocaleString()}</span>
              </p>
              <div className="popup-freq">
                <button className={`popup-freq-btn${popupFreq === 'monthly' ? ' active' : ''}`} onClick={() => setPopupFreq('monthly')}>Monthly</button>
                <button className={`popup-freq-btn${popupFreq === 'one-time' ? ' active' : ''}`} onClick={() => setPopupFreq('one-time')}>One-Time</button>
              </div>
              <div className="popup-amounts">
                {[2500, 5000, 10000].map(amt => (
                  <div key={amt} className={`popup-pill${popupAmount === amt ? ' active' : ''}`} onClick={() => { setPopupAmount(amt); setCustomAmt('') }}>₦{amt.toLocaleString()}</div>
                ))}
                <div className={`popup-pill${customAmt ? ' active' : ''}`} onClick={() => { setPopupAmount(0); setCustomAmt('') }}>Other</div>
              </div>
              <div className="popup-custom">
                <span>₦</span>
                <input type="number" placeholder="Enter other amount" value={customAmt}
                  onChange={e => { setCustomAmt(e.target.value); setPopupAmount(parseFloat(e.target.value) || 0) }} />
              </div>
              <input type="email" className="popup-email" placeholder="Your email address *" value={email} onChange={e => setEmail(e.target.value)} />
              <button className="popup-donate-btn" onClick={handleDonate}>♥ Donate Now</button>
      ...
      {/* BANK TRANSFER MODAL */}
      {bankModalOpen && (
        <div className="bank-modal-overlay" onClick={e => { if (e.target === e.currentTarget) setBankModalOpen(false) }}>
          <div className="bank-modal-inner">
            <button onClick={() => setBankModalOpen(false)} style={{ position: 'absolute', top: 16, right: 20, background: 'none', border: 'none', fontSize: 24, cursor: 'pointer', color: '#555' }}>×</button>
            ...
            <div style={{ background: '#eef3fd', border: '1.5px solid #1D56E8', borderRadius: 10, padding: 24, marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <button onClick={copyAccount} style={{ background: copied ? '#16a34a' : '#1D56E8', color: '#fff', border: 'none', borderRadius: 6, padding: '6px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                    {copied ? 'Copied!' : 'Copy'}
                  </button>
                </div>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontSize: 15, fontWeight: 700, color: '#1D56E8' }}>₦{popupAmount.toLocaleString()}</span>
              </div>
            </div>
        ...
        <div className="container">
          ...
          <div className="footer-bottom">
            ...
            <p style={{ fontSize: 11, color: '#aaa' }}>
              Built by <a href="https://www.linkedin.com/in/abiye-lawson" style={{ color: 'var(--blue)' }}>Abiye</a>
              {' & '}
